...
// function printFunc(str: string): void {
//   console.log(str);
// }
...
// function greet(fn: GreetFunction): void {
//   fn("hello world");
// }
...
// const multiply: Test1 = function (num: number): number {
//   return num * 3;
// };

// multiply.description = "Number operation";

// // let's say we create function using new keyword
...
// const map = <T, output>(arr: T[], func: (item: T) => output): output[] => {
//   return arr.map(func);
// };

// // using constraints with generics

// const checkLength = <T extends { length: number }>(a: T, b: T): T => {
...
// console.log(checkLength([1, 2, 3, 4, 5, 3, 2], [1, 2, 3, 4, 3]));

// const combine = <T>(arr1: T[], arr2: T[]): T[] => {
//   return arr1.concat(arr2);
// };

// console.log(combine([1, 3, 2, 4], [4, 3, 1]));
// console.log(combine<number | string>([1, 3, 2, 4], ["tim", "jade"]));

// Using optional parameter

function test3(n: number): void {
  if (n) {
...
function myForEach(
  arr: number[],
  callback: (num: number, index?: number | string) => void
) {
  for (let i = 0; i < arr.length; i) {
    callback(arr[i], i.toFixed());
  }
}
...
// rather than making overload choose union types
